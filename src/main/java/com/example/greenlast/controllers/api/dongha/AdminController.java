...
public class AdminController {
    ...
    @PostMapping("/approve")
    public ResponseEntity<?> approveClass(@RequestParam int classId) {
        int result = makeClassService.approveClass(classId);
        return result > 0 ? ResponseEntity.ok().body("승인 완료!") : ResponseEntity.badRequest().body("승인 실패!");
    }

    @PostMapping("/reject")
    public ResponseEntity<?> rejectClass(@RequestParam int classId, @RequestParam String reason) {
        int result = makeClassService.rejectClass(classId);
        return result > 0 ? ResponseEntity.ok().body("반려 완료!") : ResponseEntity.badRequest().body("반려 실패!");
    }
